...
def isolated_logger(mock_appender):
    ...
    with patch('managers.global_manager.GlobalManager') as mock_global:
        ...
        logger = Logger(
            name="isolated_logger",
            level=LoggingLevel.INFO,
            appenders=[mock_appender]
        )
        return logger
...
class TestLogManagerAddLogger:
    ...
    def test_add_logger_duplicate_name_raises_error(self, log_manager, sample_logger, another_logger):
        ...
        with patch('managers.global_manager.GlobalManager') as mock_global:
            duplicate_logger = Logger("test_logger", LoggingLevel.DEBUG, [ConsoleAppender(SimpleFormatter())])
        
        with pytest.raises(ValueError, match="A logger with the name 'test_logger' already exists"):
...
class TestLogManagerGetLogger:
    ...
    def test_get_logger_non_existing_raises_keyerror(self, log_manager):
        ...
        with pytest.raises(KeyError, match="No logger found with the name 'non_existing'"):
...
class TestLogManagerUtilityMethods:
    ...
    def test_get_all_loggers_with_loggers(self, log_manager, sample_logger, another_logger):
    ...
    def test_clear_loggers_with_loggers(self, log_manager, sample_logger, another_logger):
...
class TestLogManagerDictLikeOperations:
    ...
    def test_setitem_invalid_name_type_raises_typeerror(self, log_manager, sample_logger):
...
class TestLogManagerIntegration:
    ...
    def test_manager_state_independence(self, mock_appender):
        ...
        assert manager1.get_logger("shared_name") is not manager2.get_logger("shared_name")
