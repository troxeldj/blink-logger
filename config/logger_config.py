...
class LoggerConfig:
    ...
    def _parse_appenders(cls, appenders_data: List[dict]) -> List[BaseAppender]:
        ...
        for appender_data in appenders_data:
            ...
            if not isinstance(appender_instance, BaseAppender):
                raise TypeError(
                    f"Appender instance must be of type BaseAppender, got {type(appender_instance)}"
                )
    ...
    def to_logger(self):
        ...
        return Logger(name=config.name, level=config.level, appenders=config.appenders)
